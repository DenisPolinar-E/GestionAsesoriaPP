...
{
    public class GetClassificationProjectListQuery : IRequest<Result<IEnumerable<MasterDataValueResponseDto>>>
    {
    }

    internal class GetClassificationProjectListQueryHandler : IRequestHandler<GetClassificationProjectListQuery, Result<IEnumerable<MasterDataValueResponseDto>>>
    {
        private readonly IUnitOfWork<int> _unitOfWork;
        private readonly IMapper _mapper;
        private readonly IMessageService _messageService;

        public GetClassificationProjectListQueryHandler(IUnitOfWork<int> unitOfWork, IMapper mapper, IMessageService messageService)
        {
        ...
        public async Task<Result<IEnumerable<MasterDataValueResponseDto>>> Handle(GetClassificationProjectListQuery request, CancellationToken cancellationToken)
        {
            try
            {
                var masterdatavalues = await _unitOfWork.MasterDataValueRepository.GetClassificationProjectTypeListAsync();

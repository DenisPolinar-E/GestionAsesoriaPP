...
namespace GestionAsesoria.Operator.Infrastructure.Services.Identity
{
    public class RoleService : IRoleService
    {
        ...
        {
            var roles = await _roleManager.Roles.Where(x => x.IsDeleted == false && x.IsActive == true).ToListAsync();
            var rolesResponse = _mapper.Map<IEnumerable<RoleResponse>>(roles);
            return await Result<IEnumerable<RoleResponse>>.SuccessAsync(rolesResponse);
        }
        ...
        public async Task<Result<string>> SaveAsync(RoleRequest request)
        {
            if (string.IsNullOrEmpty(request.Id))
            {
                ...
                var response = await _roleManager.CreateAsync(new AcademicRole(request.Name!, request.Description!));
        ...
        public async Task<Result<string>> DeleteAsync(string id)
        {
            ...
            if (existingRole!.Name != RoleConstants.AdministratorRole && existingRole.Name != RoleConstants.BasicRole)
            {
                ...
                if (roleIsNotUsed)
                {
                    ...
                    existingRole.DeletedOn = DateTime.UtcNow;
                    existingRole.DeletedBy = _currentUserService.UserId;
                    await _roleManager.UpdateAsync(existingRole);

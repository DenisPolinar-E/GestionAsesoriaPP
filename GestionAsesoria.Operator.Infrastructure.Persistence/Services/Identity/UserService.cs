...
using GestionAsesoria.Operator.Application.Exceptions;
using GestionAsesoria.Operator.Application.Interfaces.Repositories;
using GestionAsesoria.Operator.Application.Interfaces.Services;
using GestionAsesoria.Operator.Application.Interfaces.Services.Identity;
using GestionAsesoria.Operator.Application.Validators.Requests.Identity;
...
namespace GestionAsesoria.Operator.Infrastructure.Services.Identity
{
    public class UserService : IUserService
    {
        ...
        private readonly RegisterRequestValidator _validationRules;
        ...
        public UserService(
            ...
            RegisterRequestValidator validationRules,
        ...
        public async Task<Result<List<UserResponse>>> GetAllAsync()
        {
            ...
            var result = users.Select(user => new UserResponse
            {
                ...
                Email = user.Email,
                //ReportUser = user.User.UserName,
                IsActive = user.IsActive,
                UserStatus = user.IsActive ? "Activo" : "Inactivo",
        ...
        //public async Task<Result<List<UserResponse>>> GetAllByChargeIdAsync(int chargeId)
        //{
        //    var users = await _userManager.Users.Where(x => x.ChargeId == chargeId).ToListAsync();
        //    var result = _mapper.Map<List<UserResponse>>(users);
        //    return await Result<List<UserResponse>>.SuccessAsync(result);
        //}

        public async Task<Result<List<UserSelectResponse>>> GetUserSelectAsync()
        {
            var users = await _userManager.Users.ToListAsync();
            var result = _mapper.Map<List<UserSelectResponse>>(users);
            return await Result<List<UserSelectResponse>>.SuccessAsync(result);
        }
        ...
        public async Task<IResult> RegisterAsync(RegisterRequest request)
        {
            try
            {
                ...
                {
                    return await Result.FailAsync(validationResult.Errors);
                    //return await Result.FailAsync(string.Format("Errores de validaciones."));
                }
            ...
            catch (Exception)
            {
                ...
                return await Result.FailAsync(string.Format("Algo salió mal."));
            }

        }

        private async Task<string> SendVerificationEmail(AcademicUser user, string origin)
        {
        ...
        public async Task<IResult<UserByIdResponse>> GetAsync(string userId)
        {
            var user = await _userManager.Users.Where(u => u.Id == userId).FirstOrDefaultAsync();
            var result = _mapper.Map<UserByIdResponse>(user);
            return await Result<UserByIdResponse>.SuccessAsync(result);
        }
        ...
        public async Task<IResult> ToggleUserStatusAsync(ToggleUserStatusRequest request)
        {
            ...
            if (isAdmin)
            {
                return await Result.FailAsync(ReplyMessage.MESSAGE_CHANGE_PERFIL_ADMIN_STATE);
            }
            if (user != null)
            {
            ...
            return await Result.SuccessAsync(user.IsActive == true ? ReplyMessage.MESSAGE_ACTIVO : ReplyMessage.MESSAGE_DESACTIVO);
        }
